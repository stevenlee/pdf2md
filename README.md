...
1. **第一階段：物理萃取** (marker-pdf) - 高速解析 PDF 文字、公式與圖片，產出 `_raw.md`。
2. **第二階段：語意增強** (Async Enhancer) - **並發**呼叫 LLM 將圖表轉換為 Mermaid 語法，並將表格圖片 OCR 成 Markdown table。
...
## ✨ 特色功能：Mermaid 語法優化
為了確保圖表在 Obsidian 中完美的渲染，本專案實作了多層次的 Mermaid 語法清洗：
- **自動化標籤包覆**：強制為所有節點標籤加上雙引號，即使標籤內包含括號（如 `(S100)`）也能正常解析。
- **巢狀引號處理**：自動將標籤內部的雙引號轉義為單引號，避免 `ID["Text "Inside""]` 導致的語法錯誤。
- **語意精準修復**：採用「上下文感知」的正則引擎，僅修正圖表標籤，不影響 Mermaid 的連線語法（如 `-->`）。

## ✨ 特色功能：表格圖片轉 Markdown table
- **表格圖片 OCR**：遇到 PDF 萃取出的表格截圖時，會先嘗試轉成 GitHub-Flavored Markdown table。
- **直接吃圖片檔**：`input_dir/` 可直接放 `.jpg`、`.jpeg`、`.png`、`.webp`、`.tif`、`.tiff`，會輸出同名 Markdown。
- **保留原圖比對**：轉換後仍會附上原始圖片連結，方便人工校對。
- **可關閉**：若只想保留圖片，可於 CLI 使用 `--no-tables`。
